...
resource "apple-ads_ad_group" "test" {
...
`, namePrefix),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrSet("apple-ads_ad_group.test", "id"),
					resource.TestCheckResourceAttr("apple-ads_ad_group.test", "name", namePrefix+"-create"),
					resource.TestCheckResourceAttr("apple-ads_ad_group.test", "status", "PAUSED"),
					resource.TestCheckResourceAttr("apple-ads_ad_group.test", "default_bid_amount", "1.00"),
					resource.TestCheckResourceAttrPair("apple-ads_ad_group.test", "campaign_id", "apple-ads_campaign.parent", "id"),
				),
			},
			{
				Config: testAccProviderConfig(true) + testAccCampaignPausedConfig("parent", adamID) + fmt.Sprintf(`
...
`, namePrefix),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("apple-ads_ad_group.test", "name", namePrefix+"-updated"),
					resource.TestCheckResourceAttr("apple-ads_ad_group.test", "default_bid_amount", "1.50"),
					resource.TestCheckResourceAttr("apple-ads_ad_group.test", "automated_keywords_opt_in", "true"),
				),
			},
			{
				ResourceName:      "apple-ads_ad_group.test",
				ImportState:       true,
				ImportStateVerify: true,
				ImportStateIdFunc: func(s *terraform.State) (string, error) {
					rs := s.RootModule().Resources["apple-ads_ad_group.test"]
...
resource "apple-ads_ad_group" "immutable" {
  campaign_id          = apple-ads_campaign.a.id
...
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrSet("apple-ads_ad_group.immutable", "id"),
					func(s *terraform.State) error {
						rs := s.RootModule().Resources["apple-ads_ad_group.immutable"]
			...
			},
			{
				Config: testAccProviderConfig(true) + testAccCampaignPausedConfig("a", adamID) + testAccCampaignPausedConfig("b", adamID) + `
resource "apple-ads_ad_group" "immutable" {
  campaign_id          = apple-ads_campaign.b.id
  name                 = "tf-acc-adgroup-immutable"
  status               = "PAUSED"
  default_bid_amount   = "1.00"
  default_bid_currency = "USD"
...
func testAccCampaignPausedConfig(name, adamID string) string {
	return fmt.Sprintf(`
resource "apple-ads_campaign" "%[1]s" {
