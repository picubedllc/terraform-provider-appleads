...
resource "appleads_campaign" "test" {
  name                 = "%[1]s-create"
  adam_id              = "%[2]s"
...
`, namePrefix, adamID),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrSet("appleads_campaign.test", "id"),
...
`, namePrefix, adamID),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("appleads_campaign.test", "name", namePrefix+"-updated"),
					resource.TestCheckResourceAttr("appleads_campaign.test", "daily_budget_amount", "2.00"),
				),
			},
			// Import
			{
				ResourceName:      "appleads_campaign.test",
				ImportState:       true,
...
resource "appleads_campaign" "protected" {
...
`, adamID),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrSet("appleads_campaign.immutable", "id"),
					func(s *terraform.State) error {
						rs := s.RootModule().Resources["appleads_campaign.immutable"]
