...
When `allow_campaign_deletion` is `false` (the default), any attempt to destroy/archive an `apple-ads_campaign` returns a clear error diagnostic and does not call Apple's delete API.
